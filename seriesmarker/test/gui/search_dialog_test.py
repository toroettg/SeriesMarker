...
class SearchDialogTest(GUITestCase):
    def setUp(self):
        ...
        self.assertEqual(self.dialog.ui.search_text_field.text(), "", "Default Search Text is not empty")
        self.assertEqual(self.dialog.model.rowCount(), 0, "Result model not cleared")
    ...
    def test_clear_on_new_search(self):
        ...
        for i in range(len(self.dialog.ui.search_text_field.text())):  # @UnusedVariable
            self.keyhit(self.dialog.ui.search_text_field, Qt.Key_Backspace)

        self.assertEqual(self.dialog.ui.search_text_field.text(), "", "Emulated typing did not delete all characters")

        self.click(self.dialog.ui.search_button)

        self.assertEqual(self.dialog.model.rowCount(), 0, "Search did not clear model")

    def test_reject(self):
        self.click(self.dialog.ui.cancel_button)

        self.assertEqual(self.dialog.result(), self.dialog.Rejected, "Dialog did not reject")

    def test_accept(self):
        self.click(self.dialog.ui.ok_button)
        self.assertEqual(self.dialog.result(), self.dialog.Rejected, "Dialog did accept without search or selection")
        self.assertEqual(self.dialog.result_value(), None, "Accept without search changed result")
        ...
        self.click(self.dialog.ui.ok_button)
        self.assertEqual(self.dialog.result(), self.dialog.Rejected, "Dialog did accept without selection")
        self.assertEqual(self.dialog.result_value(), None, "Accept without selection changed result")
        ...
        self.assertEqual(self.dialog.result(), self.dialog.Accepted, "Dialog did not accept")
        self.assertEqual(self.dialog.result_value().SeriesName, "How I Met Your Mother", "Accept with selection did not return correct search result")

    def tearDown(self):
        QTest.mouseMove(self.dialog, delay=2000)  # Emulates waiting, can be removed
